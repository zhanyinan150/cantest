...
*** ID_Addr  : 1~2          (电机地址, 与代码 CHASSIS_LEFT_ADDR/RIGHT_ADDR 一致)
*** CAN_Baud : 1000000      (1Mbps, 须与主控CAN2波特率一致)
*** Checksum : 0x6B         (固定校验字节, 命令末尾均为0x6B)
*** Response : Receive      (只返回确认收到; 到位返回需设Reached/Both)
*** Cal      : 首次上电空载校准 (否则位置闭环误差 ±0.75°~1.5°)
***
*** ===== CAN通讯协议要点 (说明书 6.4 / 6.2.1) =====
*** 帧类型   : 扩展帧 (use_ext_id=1)
*** ExtId    : (Addr << 8) | Packet, Addr=电机地址, Packet=分包序号(从0)
*** 分包     : 命令>8字节时拆包, 每包≤8字节, Packet从0递增; 每包首字节为功能码
***             (地址在ExtId里, 不在数据区; 串口版才在数据区含地址字节)
*** 校验     : 固定0x6B (末字节)
*** 速度范围 : 0~3000 RPM (F6速度模式, 0x0BB8; >3000驱动器返回E2参数错误)
*** 位置脉冲 : FD命令clk单位=细分脉冲, 16细分下3200脉冲/圈 (非编码器值65536!)
*** 加速度   : 0~255档, 0=直接启动; 公式 t=(256-acc)*50us 每1RPM
*** 运动模式 : FD命令raF: 00=相对上次目标 01=绝对坐标 (Emm42_V5.0无02相对当前)
**********************************************************/
